...
# Project Context: To-Do Application (MVP)
...
```bash
npm start          # dev server at http://localhost:4200 (auto-reloads on save)
npm run build      # production build → dist/todo/
npm run watch      # dev build in watch mode
npm test           # run unit tests with Karma/Jasmine (opens browser)
```

Scaffold new pieces with the Angular CLI:
```bash
...
Run a single spec file by passing `--include` to Karma or isolating with `fdescribe`/`fit` in the spec.

## Architecture

Angular 19 standalone-component app — **no NgModules**. Every component, directive, and pipe must list its own `imports` array rather than relying on a shared module.

- `src/main.ts` — bootstraps `AppComponent` with `appConfig`
- `src/app/app.config.ts` — application-level providers (`provideRouter`, `provideZoneChangeDetection`)
- `src/app/app.routes.ts` — route table (currently empty; add feature routes here)
- `src/app/app.component.*` — root shell; currently shows the Angular default welcome page (placeholder to be replaced)

Change detection uses Zone.js with `eventCoalescing: true`. The build target is `@angular-devkit/build-angular:application` (the newer Vite/esbuild-backed builder, not the legacy Webpack one).

Global styles live in `src/styles.css`; component-scoped styles go in the component's own `.css` file. Static assets placed in `public/` are served at the root path.
